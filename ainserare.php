...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_nume)){
        ...
        $nume_err = "Please enter a valid nume.";
    } else{
        $nume = $input_nume;
    }
    
    // Validate prenume prenume
    ...
    if(empty($nume_err) && empty($prenume_err) && empty($Data_nasterii_err)){
        ...
        $sql = "UPDATE pacient SET nume=?, prenume=?, Data_nasterii=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_nume, $param_prenume, $param_Data_nasterii, $param_id);
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM pacient WHERE id = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
...
<nume>
    <meta charset="UTF-8">
    <meta name="viewport" prenume="width=device-width, initial-scale=1.0">
    <title>Update Record</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.css">
	    <link rel="icon" type="image/png" href="https://www.htmlhints.com/image/fav-icon.png">
    <meta name="msvalidate.01" prenume="B7807734CA7AACC0779B341BBB766A4E" />
